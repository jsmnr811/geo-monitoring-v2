<div class="p-4 md:p-6 lg:p-8 space-y-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
            ...
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-3">
                    ...
                    @if($progressOnlyMode)
                    <span class="px-2 py-1 text-xs rounded-md bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300">
                        Progress Only (30/70)
                    </span>
                    @else
                    <span class="px-2 py-1 text-xs rounded-md bg-gray-100 text-gray-800 dark:bg-zinc-700 dark:text-white">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        ...
        <div class="flex flex-col items-end gap-1">
            ...
            {{-- TOTAL SCORE --}}
            <div class="px-4 py-2 rounded-lg text-base font-bold shadow-sm border
                {{ $totalScore >= 90
    ...
    {{-- SCORE BREAKDOWN --}}
    <div class="mt-4 pt-4 border-t border-gray-200 dark:border-zinc-700 text-sm space-y-3">

        {{-- ✅ BASIC COMPONENTS (HIDDEN IN PROGRESS ONLY MODE) --}}
        @if(!$progressOnlyMode)
        <div class="flex flex-col sm:flex-row sm:justify-between gap-2 py-2">

            <span class="font-medium text-gray-900 dark:text-white">
                Basic Components (Based Photos {{ $basedPhotosWeight }}%
                {{ !empty($applicable['completed_album']) ? ', Completed Album 10%' : '' }}):
            </span>

            <span class="px-3 py-1 rounded-lg font-semibold text-sm border
                {{ $basicScore > 0
                    ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300'
                    : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' }}">
                {{ number_format($basicScore, 2) }}%
            </span>
